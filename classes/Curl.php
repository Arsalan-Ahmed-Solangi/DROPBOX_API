<?php 
	...
	class Curl
	{
		...
		public function post($url,$data = array(),$headers = array())
		{
			curl_setopt($this->init,CURLOPT_URL ,$url);
			curl_setopt($this->init,CURLOPT_SSL_VERIFYPEER,false);
			curl_setopt($this->init, CURLOPT_CAINFO, "cacert.pem");
			curl_setopt($this->init, CURLOPT_SSL_VERIFYHOST, 0);
		...
		public function file($url, $headers,$file,$size)
		{
			curl_setopt($this->init, CURLOPT_HTTPHEADER, $headers);
			curl_setopt($this->init, CURLOPT_PUT, true);
			curl_setopt($this->init, CURLOPT_CUSTOMREQUEST, 'POST');
			curl_setopt($this->init, CURLOPT_INFILE, $file);
			curl_setopt($this->init, CURLOPT_INFILESIZE, $size);
			curl_setopt($this->init, CURLOPT_RETURNTRANSFER, true);
